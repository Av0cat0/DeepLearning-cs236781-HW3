...
def char_maps(text: str):
    ...
    # ====== YOUR CODE: ======
    sorted_set = sorted(set(text)) # find unique chars and sort them with ascii
    char_to_idx = {x:sorted_set.index(x) for x in text}
...
def chars_to_onehot(text: str, char_to_idx: dict) -> Tensor:
    ...
    # ====== YOUR CODE: ======
    result = torch.zeros(len(text), len(char_to_idx))
    idx_list = [char_to_idx[x] for x in text]
...
def onehot_to_chars(embedded_text: Tensor, idx_to_char: dict) -> str:
    ...
    # ====== YOUR CODE: ======
    idx = embedded_text.nonzero()
    result=""
    result = result.join([idx_to_char[idx[x,1].item()] for x in idx[:,0]])
    # ========================
    return result
...
def chars_to_labelled_samples(text: str, char_to_idx: dict, seq_len: int, device="cpu"):
    ...
    # ====== YOUR CODE: ======
    num = (len(text)-1)//seq_len
    text_onehot = chars_to_onehot(text[:num*seq_len], char_to_idx)
    samples = torch.reshape(text_onehot, (num, seq_len, text_onehot.shape[-1]))
    
    onehot_labels = chars_to_onehot(text[1:num*seq_len+1], char_to_idx)
    labels = torch.reshape(onehot_labels, (num, seq_len, text_onehot.shape[-1]))
    labels = labels.nonzero()[:,-1].reshape(num, seq_len)
...
def hot_softmax(y, dim=0, temperature=1.0):
    ...
    # ====== YOUR CODE: ======
    if temperature == 0:
        raise ValueError("Temperature must be a non-zero number")
...
class SequenceBatchSampler(torch.utils.data.Sampler):
    ...
    def __iter__(self) -> Iterator[int]:
        ...
        for i in range(num_batches):
            # Modulo logic
            curr_batch = [indices for indices in range(i, last_batch_index, num_batches)]
            idx += curr_batch
        # ========================
        return iter(idx)
