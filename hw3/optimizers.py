...
class MomentumSGD(Optimizer):
    ...
    def step(self):
        for p, dp in self.params:
            ...
            # raise NotImplementedError()
            dp += self.reg * p
            self.v[self.idx] = self.momentum * self.v[self.idx] - self.learn_rate * dp
            p += self.v[self.idx]
            self.idx += 1
        self.idx = 0
            # ========================
...
class RMSProp(Optimizer):
    def __init__(self, params, learn_rate=1e-3, reg=0, decay=0.99, eps=1e-8):
        ...
        # ====== YOUR CODE: ======
        # raise NotImplementedError()
        self.r = [torch.zeros_like(dp) for p, dp in params]
        self.idx = 0
        # ========================
    ...
    def step(self):
        for p, dp in self.params:
            ...
            # raise NotImplementedError()
            dp += self.reg * p
            self.r[self.idx] = self.decay * self.r[self.idx] + (1-self.decay) * dp**2
            p -= (self.learn_rate / torch.sqrt(self.r[self.idx] + self.eps)) * dp
            self.idx += 1
        self.idx = 0
            # ========================
