...
class TanH(Layer):
    ...
    def backward(self, dout):
        ...
        # ====== YOUR CODE: ======
        out = self.grad_cache['x']
        dx=(torch.ones_like(out) - pow(out,2))*dout
        # ========================
...
class Linear(Layer):
    ...
    def __init__(self, in_features, out_features, wstd=0.1):
        ...
        # ====== YOUR CODE: ======
        # self.w = torch.normal(wstd,torch.zeros(out_features, in_features))
        # self.b = torch.normal(wstd, torch.zeros(out_features))
...
class CrossEntropyLoss(Layer):
    ...
    def forward(self, x, y):
        ...
        # ====== YOUR CODE: ======
        x_scores = x[range(N), y]
        losses = -x_scores + torch.log(torch.sum(torch.exp(x), dim=1))
        loss = losses.mean()
        # ========================
    ...
    def backward(self, dout=1.0):
        ...
        softmax = torch.div(torch.exp(x), exp_sum)
        softmax[range(N), y] -= 1
        dx = torch.div(dout * softmax, N)
        # ========================
...
class Dropout(Layer):
    ...
    def forward(self, x, **kw):
        ...
        if self.training_mode:
            ...
            out = x * mask * scaling
            self.grad_cache["mask"] = mask * scaling
        else:
            out = x
        # ========================
...
class MLP(Layer):
    ...
    def __init__(
        ...
        **kw,
        ...
        layers.append(Linear(in_features, hidden_features[0], **kw))
        layers.append(act_func[activation]())
        if dropout:
            layers.append(Dropout(dropout))

        for l in range(len(hidden_features)-1):
            layers.append(Linear(hidden_features[l],hidden_features[l+1], **kw))
            layers.append(act_func[activation]())
            if dropout:
                layers.append(Dropout(dropout))
